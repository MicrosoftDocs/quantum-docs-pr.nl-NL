---
uid: Microsoft.Quantum.Math.AbsSquaredComplexPolar
title: De functie AbsSquaredComplexPolar
ms.date: 10/26/2020 12:00:00 AM
ms.topic: article
qsharp.kind: function
qsharp.namespace: Microsoft.Quantum.Math
qsharp.name: AbsSquaredComplexPolar
qsharp.summary: Returns the squared absolute value of a complex number of type `ComplexPolar`.
ms.openlocfilehash: 6157148e9f4da8b3fafed5c2fb5d8956ba47b037
ms.sourcegitcommit: 29e0d88a30e4166fa580132124b0eb57e1f0e986
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 10/27/2020
ms.locfileid: "92706492"
---
# <a name="abssquaredcomplexpolar-function"></a>De functie AbsSquaredComplexPolar

Naam ruimte: [micro soft. Quantum. math](xref:Microsoft.Quantum.Math)

Pakket [](https://nuget.org/packages/)


Retourneert de kwadraat absolute waarde van een complex type `ComplexPolar` .

```qsharp
function AbsSquaredComplexPolar (input : Microsoft.Quantum.Math.ComplexPolar) : Double
```


## <a name="input"></a>Invoer

### <a name="input--complexpolar"></a>invoer: [ComplexPolar](xref:Microsoft.Quantum.Math.ComplexPolar)

Complex getal $c = r e ^ {i t} $.



## <a name="output--double"></a>Uitvoer: [dubbel](xref:microsoft.quantum.lang-ref.double)

Absolute waarde in kwadraat $ | c | ^ 2 = r ^ 2 $.