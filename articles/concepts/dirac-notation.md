...
Meer in het algemeen geldt dat als $ \psi $ en $ \phi $ Quantum State vectoren zijn, het binnenste product $ \braket { \phi | \psi $ is. } Dit impliceert dat de kans op het meten van de status $ \ket { \psi } $ tot $ \ket { \phi } $ $ | \braket { \phi | \psi } | ^ 2 is $ .  
...
De volgende notatie wordt vaak gebruikt voor het beschrijven van de statussen die het gevolg zijn van het Toep assen van de Hadamard-poort op $ \ket{0 } $ en $ \ket{1 } $ (die overeenkomt met de eenheids vectoren in de $ + x $ -en $-x- $ richting op de Bloch-bol):
...
U vraagt zich misschien af waarom de som van $0 $ tot $2 ^ {n } -1 $ voor $n $ bits gaat.  Eerst moet u er rekening mee houden dat er $2 ^ {n } $ verschillende configuraties zijn die $n $ bits kunnen uitvoeren.  U kunt dit zien door aan te geven dat één bit $2 waarden kan hebben $ , maar twee bits waarden van $4 kunnen hebben $ , enzovoort. In het algemeen betekent dit dat er $2 ^ n $ andere mogelijke bits teken reeksen zijn, maar de grootste waarde die is gecodeerd in een van deze $1 \cdots 1 = 2 ^ n-1 $ en daarom de bovengrens voor de som is.
Als kantlijn notitie hebben we in dit voor beeld geen gebruik gemaakt van $ \ket { +} ^ {\otimes n } = \ket { +} $ in analoog naar $ \ket{0 } ^ {\otimes n } = \ket{0 } $, omdat deze notatie Conventie doorgaans is gereserveerd voor de status van de reken kundige basis waarbij elke qubit is geïnitialiseerd op nul.  Hoewel een dergelijke Conventie in dit geval verstandig is, wordt deze niet in de Quantum Computing-documentatie gebruikt.
...
Stel dat we de kans willen berekenen dat de status wordt gemeten $ \ket { \psi } = \frac{3 } {5 } \ket{1 } + \frac{4 } {5 } \ket{0 } $ met behulp van een Quantum programma om de status te meten als $ \ket { +} $ of $ \ket { -} $. Vervolgens wordt de kans dat het apparaat wordt uitgevoerd dat de status $ \ket { -} $ is 
