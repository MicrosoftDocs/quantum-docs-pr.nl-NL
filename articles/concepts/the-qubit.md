---
title: De Qubit | Microsoft Docs
description: De Qubit
...
Houd er rekening mee dat de Conventie voor het vermenigvuldigen van de matrix van rechts naar links de eerste poort bewerking in deze reeks is, $G _N $, in werkelijkheid de laatste is die is toegepast op de Quantum status vector. Formeel, we zeggen dat een dergelijke Gate set universeel is als voor elke fout tolerantie $ \epsilon > 0 $ bestaat $G _1, \ldots, G_N $, waardoor de afstand tussen $G _N \ ldots G_1 $ en $U $ Maxi maal $ \epsilon $ is. In het ideale geval de waarde van $N $ die nodig is om deze afstand van $ \epsilon $ te bereiken, moet poly-logarithmically met $1/\ Epsilon $ worden geschaald.
...
Als voor beeld van hoe unitary-trans formaties kunnen worden opgebouwd uit deze primitieven, komen de drie trans formaties in de bovenstaande Bloch-gebieden overeen met de poort reeks $ \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} \mapsto HZH \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} = \begin{bmatrix} 0 \\\\ 1 \end{bmatrix} $.
...
De eenvoudigste manier om een dergelijke primitieve te zijn, is de enkele Qubit. Drie rotaties met één Qubit worden doorgaans beschouwd: $R _x $, $R _y $ en $R _z $. Als u de actie van de draaiing $R _x (\theta) $ wilt visualiseren, kunt u bijvoorbeeld een voor beeld geven van de rechter muisknop langs de richting van de $x $-as van de Bloch-bol en de vector met uw hand in een hoek van $ \ theta/2 $ radialen draaien. Deze onduidelijke factor van $2 $ ontstaat uit het feit dat de rechthoekige vectoren $180 ^ \circ $ van elkaar worden weer gegeven op de Bloch-bol, maar eigenlijk $90 ^ \circ $ graden onafhankelijk van elkaar liggen. De bijbehorende unitary-matrices zijn:
