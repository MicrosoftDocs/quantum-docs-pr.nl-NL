...
We zijn enthousiast om de nieuwste update uit te voeren op de Quantum Development Kit.  Op deze pagina vindt u een overzicht van de nieuwe Q #-taal functies en wordt beschreven hoe u bestaande Q #-Program ma's [migreert](#Migration) naar de nieuwe syntaxis.  Zie onze volledige documentatie voor de volledige beschrijving van de Q #-taal [hier](xref:microsoft.quantum.guide.basics).
...
Een voorwaardelijke expressie is een expressie van het formulier `condition ? caseTrue | caseFalse` .
Zoals in het bovenstaande voor beeld kan worden weer gegeven, wordt iteratie over matrices nu ook ondersteund.

Toewijzingen binnen `using` en `borrowing` zijn nieuw mogelijk voor één qubits, Qubit-matrices en geneste Tuples daarvan:
```qsharp
    using (qubit = Qubit()) {
...
Er zijn twee nieuwe richt lijnen voor het maken van een specialize-Generator `invert` en `distribute` `auto` zijn geïntroduceerd om de controle te verg Roten over hoe functor specialize worden gegenereerd.
Een expliciete declaratie van de standaard `body` specialisatie in bewerkingen is niet meer verplicht.
...
`Adjoint`, `Controlled` en een nieuwe achtervoegsel-operator ' uitpakken ' `!` worden behandeld als expressie parameters. `Adjoint`Als u `Controlled` de para meters voor expressies behandelt, worden er geen haakjes nodig voor een groot aantal cases. De onderstaande expressies zijn bijvoorbeeld geldig voor `Op` , een unitary-bewerking, `opArr` een matrix van unitary-bewerkingen en `arg` een geschikt argument: 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
Het migratie script verwijdert het meren deel van het overbodige haakje en profiteert optimaal van de nieuwe functies. U wordt echter geadviseerd dat het script de code niet interpreteert en geen account voor de verschillende interpretatie van de `Controlled` functor in zeldzame gevallen (alleen relevant in combi natie met gedeeltelijke toepassingen).

Met de nieuwe wijzigings functie wordt een door de `!` gebruiker gedefinieerd type naar het onderliggende type gecast. Deze cast is als een upcast in eerdere releases beschouwd en is automatisch uitgevoerd. Met de nieuwe interpretatie van door de gebruiker gedefinieerde typen is dit niet langer het geval en moet de cast expliciet worden gemaakt.    
...
```
Zoals u kunt zien in het voor beeld, `!` bindt u nauw keuriger dan de voorvoegsel opties, zoals `Controlled` en `Adjoint` .

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>Wijzigingen afbreken 
...
Naast de hieronder vermelde wijzigingen wordt het patroon `__*__` waarbij * een wille keurige reeks tekens voor niet-witruimte is gereserveerd voor intern gebruik en kan niet worden gebruikt als symbool naam.  
...
In het volgende voor beeld ziet u hoe u de functie kunt gebruiken `ToOperation` om een functie `Square` te wijzigen in een bewerking `op` : 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>Bestaande projecten migreren naar Q # 0,3 
...
2. Bewerk in een tekst editor het. csproj-bestand om de versie van alle ' micro soft. Quantum ' `PackageReference` te wijzigen in versie 0.3.1811.2802-preview, bijvoorbeeld:
```xml
...
## <a name="migration-script"></a><a name="MigrationScript"></a>Migratie script 
...
- Het project is gebouwd met de vorige versie van de Quantum Development Kit om ervoor te zorgen dat de migratie waarschijnlijk slaagt. Deze stap kan worden overgeslagen met behulp van de `-Force` para meter voor het script.
