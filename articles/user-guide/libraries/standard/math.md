---
title: 'Wiskunde in de :::no-loc(Q#)::: standaard bibliotheken'
description: 'Meer informatie over de klassieke wiskundige functies in de :::no-loc(Q#)::: standaard bibliotheken die worden gebruikt met de ingebouwde gegevens typen.'
author: cgranade
...
<span data-ttu-id="80288-104">Deze functies worden voornamelijk gebruikt voor het gebruik van de :::no-loc(Q#)::: ingebouwde gegevens typen `Int` , `Double` en `Range` .</span><span class="sxs-lookup"><span data-stu-id="80288-104">These functions are primarily used to work with the :::no-loc(Q#)::: built-in data types `Int`, `Double`, and `Range`.</span></span>
