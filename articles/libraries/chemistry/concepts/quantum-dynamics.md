...
Daarom kunnen we de oorzaken van de mogelijke energie van het systeem vaak uitdrukken, waardoor we $-\partial_x V (x, t) = m \frac{\dd ^ 2} {\dd t ^ 2} {x} (t) $ hebben.
...
\end{align} hier $\\{f, H\\} $ wordt de [Poisson-haak](https://en.wikipedia.org/wiki/Poisson_bracket) genoemd en wordt weer gegeven in de klassieke dynamiek als gevolg van de centrale rol die wordt afgespeeld bij het definiëren van Dynamics.
...
Zodra we de Hamiltonian-operator hebben gemaakt door de gewone klassieke Hamiltonian te vertalen naar de Quantum taal, kunnen we de dynamiek van een wille keurige Quantum hoeveelheid (bijvoorbeeld quantum mechanisch-operator) als volgt uitdrukken: $ \hat{f} (t) $ via \begin{ align} \frac{d}{DT} \hat{f} = \partial_t \hat{f} + [\hat{f}, \hat{H}], \end{align} waarbij $ [f, H] = fH-HF $ bekend staat als de commutator.
Deze expressie is precies hetzelfde als de klassieke expressie die hierboven wordt gegeven, waarbij het verschil is dat het Poisson-haakje $\\{f, H\\} $ wordt vervangen door de commutator tussen $f $ en $H $.
