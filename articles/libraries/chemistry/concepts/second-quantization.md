...
In plaats van elke $N _e $ electrons toe te wijzen aan een specifieke staat (of Orbital), houdt tweede kwantisatiefouten elke Orbital bij en slaat het op of er in elk van deze elektron aanwezig zijn en tegelijkertijd automatisch de symmetrie-eigenschappen van de overeenkomende functie Wave.
...
Dat wil zeggen dat de geldige basis statussen de volgende notatie hebben: $ \ psi_ {0, \uparrow}, \ldots, \ psi_ {N-1, \uparrow}, \ psi_ {0, \downarrow}, \ldots, \ psi_ {N-1, \downarrow} $, waarbij $ \uparrow $ en $ \downarrow $ de labels zijn die de twee eigenstates van de kring waarde van vrijheid.
...
Dit betekent dat we formeel kunnen nadenken over het spin en ruimtelijke deel van de functie Wave als $ \ psi_{0} \cdots \ psi_ {2N-1} $, waarbij elk van de indices nu een opsomming is van $ (j, \sigma) $.
...
Met behulp van dergelijke Opera tors kunnen we ook de \ket van $ $ ${0} \ket{1} \ket{1} \ket{0} = a ^ \ dagger_1 een ^ \ dagger_2 \ket{0}^ {\otimes 4}.
...
Voor een systeem van $k $ Fermions, in de tweede kwantisatiefouten is de actie van de operator maken $a ^ \ dagger_i $ gegeven door $ $ a ^ \ dagger_i \ket{n_1, n_2, \ldots, 0_i, \ldots, n_k} = (-1) ^ {S_i} \ket{n_1, n_2, \ldots 1_i , \ldots, n_k}, $ $ en $ $ a ^ \ dagger_i \ket{n_1, n_2, \ldots, 1_i, \ldots, n_k} = 0, $ $ waarbij $S _i = \ sum_ {j < i} a ^ \ dagger_j a_j $ meet het totale aantal Fermions dat in de staat van één partikel is en die een index heeft $j < i $.
...
\end{Equation} deze operator telt het beroep van een gegeven Orbital, wat wil zeggen \begin{align} n_i \ket{0}_i & = 0 \\\\
...
Dit betekent dat $ $ a ^ \ dagger_2 a ^ \ dagger_1 \ket{0} =-a ^ \ dagger_1 een ^ \ dagger_2 \ket-{0}.
...
\begin{Equation} \hat{H} = \sum\_{pq} H\_{pq} a ^ \dagger\_p a\_q + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_p a ^ \dagger\_q a\_Ra\_s + H\_{\textrm NUC}, \label{EQ: totalHam} \end{Equation} waarbij $h\_{\textrm NUC} $ de kern energie is (wat een constante is onder de in de lucht-Oppenheimer benadering) en
...
\begin{align} h\_{pqrs} & = \int\_{-\infty} ^ \infty \int\_{-\infty} ^ \infty\psi\_p ^\*(x\_1) \psi\_q ^\*(x\_2) \left (\frac{1}{| x_1-x_2 |} \right) \psi\_r (x\_2) \psi\_s (x\_1) \mathrm{d} ^ 3x\_1 \ mathrm {d} ^ 3x\_2. \ label {EQ : integralen} \end{align}

De voor waarden $h\_{pq} $ worden aangeduid als een enkele-elektro-integraal, omdat al deze voor waarden slechts één electrons omvatten en ook $h\_{pqrs} $ de twee-elektroden integraal zijn.
...
$ $ \begin{align} H = \sum\_{pq} H\_{pq} a ^ \dagger\_{p} a\_{q} + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} een\_{s}.
\end{align} $ $
...
Bijvoorbeeld als $p, q, r, s $ zijn afzonderlijke indices. we kunnen de anti-werk regels gebruiken om aan te tonen dat: $ $ a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} =-a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{r} a\_{s} =-a ^ \dagger\_p} a ^ \dagger\_{q} a\_{s} a\_{r} = a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{s} a\_{r}.
$$

Als $H $ is Hermitian, moet elke niet-Hermitian fermionic-operator bijvoorbeeld $h\_{pqrs} a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} $, een Hermitian geconjugeerde is die ook is gevonden in $H $. Als u unieke groepen termen wilt indexeren die door deze Symmetries worden gekenmerkt, definieert u een canonieke volg orde op de indices $ (i\_1, \cdots, i\_n, j\_1, \cdots, j\_m) $ van een reeks van $n + m $ fermionic Opera tors $a ^ \dagger\_{i\_1} \cdots a ^ \dagger\_{i\_n}\_{j\_1} \cdots a\_{j\_m} $as volgt :
...
Met deze canonieke ordening kan de fermionic-Hamiltonian worden uitgedrukt als $ $ \begin{align} H = \sum\_{(p, q) \in S\_{1,1}} H '\_{pq} \frac{a ^ \dagger\_{p} a\_{q} + a ^ \dagger\_{q} a\_{p}}{2}+ \sum\_{(p, v, r, S) \in S\_{2,2}} H '\_{pqrs} \frac{a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} + a ^ \dagger\_{s} a ^ \ Dagger\_{r} a\_{q} a\_{p}}{2}, \end{align} $ $ met voldoende aangepaste integraals van één en twee elektroden $h '\_{pq} $ en $h '\_{pqrs} $, respectievelijk.
