...
In het bijzonder laten we $H = \sum_{j = 1} ^ m H_j $ de Hamiltonian zijn.
Vervolgens $ $ e ^ {-i\sum_ {j = 1} ^ m H_j t} = \prod_{j = 1} ^ m e ^ {-iH_j t} + O (m ^ 2 t ^ 2), $ $, wat wil zeggen dat, als $t \ll $1, de fout in deze benadering verwaarloosbaar wordt.
Als $e ^ {-i H t} $ een normale exponentiële waarde was, zou de fout in deze benadering niet worden $O (m ^ 2 t ^ 2) $: de waarde is nul.
Deze fout treedt op omdat $e ^ {-iHt} $ een operator exponentiële is. als gevolg hiervan is er een fout opgetreden bij het gebruik van deze formule als gevolg van het feit dat de $H _J $-voor waarden niet werken (*dat wil zeggen*, $H _J H_k \Ne H_k H_j $ in het algemeen).
...
Vervolgens hebben we dat $ $ e ^ {-i\sum_ {j = 1} ^ m H_j t} = \left (\prod_{j = 1} ^ m e ^ {-iH_j t/r} \ right) ^ r + O (m ^ 2 t ^ 2/r), $ $, wat impliceert dat als $r $ wordt geschaald als $m ^ 2 t ^ 2/\ Epsilon $, de fout kan worden gemaakt Maxi maal $ \epsilon $ voor elke $ \epsilon > 0 $.
...
De eenvoudigste van deze formule, de symmetrische Trotter formule of Strang splitsen, heeft de vorm $ $ U_1 (t) = \prod_{j = 1} ^ m e ^ {-iH_j t/2} \ prod_ {j = m} ^ 1 e ^ {-iH_j t} = e ^ {-iHt} + O (m ^ 3 t ^ 3), $ $ die minder dan $ \epsilon $ kan worden gemaakt voor $ \epsilon > 0 $ door $ te kiezen r $ om in te schalen als $m ^ {3/2} t ^ {3/2}/\sqrt {\ Epsilon} $.
...
Exponentiële waarde van Pauli-Opera tors kunnen rechtstreeks in Q # worden geïmplementeerd met behulp van de <xref:microsoft.quantum.primitive.exp> bewerking:
```qsharp
...
Deze indeling van de Jordanië – Wigner reprsentation die wordt verbruikt door de Q #-simulatie algoritmen is een door de gebruiker gedefinieerd type `JordanWignerEncodingData`.
...
De volgende stap omvat het transformeren van de eigenvalues van de Walk-operator van $e ^ {i\pm \cos ^{-1}(E_k/| h | _1)} $, waarbij $E _k $ de eigenvalues is van $H $ tot $e ^ {-iE_k t} $.
...
De eerste Quantum subroutine die qubitization gebruikt, heet $ \operatorname{Select} $ en is toegezegd om \begin{Equation} \operatorname{Select} \ket{j} \ket{\psi} = \ket{j} H_j \ket{\psi}, \end{Equation} te leveren waarbij elke $H _J $ wordt verondersteld Hermitian te zijn en unitary.
...
De $ \operatorname{Prepare} $-bewerking wordt niet rechtstreeks in qubitization gebruikt, maar wordt gebruikt voor het implementeren van een reflectie over de status die $ \operatorname{Prepare} $ $ $ \begin{align} R &amp; = 1-2 \ operatornaam {Prepare} \ket{0}\bra @no_ _t_2_ \operatorname{Prepare} ^{-1} \\\\ &amp; = \operatorname{Prepare} \Lambda \operatorname{Prepare} ^{-1}.
\end{align} $ $
...
Een voor beeld van het simuleren van het Hubbard model kan worden gevonden als een [Q #](https://github.com/Microsoft/Quantum/tree/master/Samples/src/HubbardSimulation)-voor beeld.
