...
Quantum encoding en verwerking van informatie is een krachtig alternatief voor klassieke machine learning Quantum-classificaties, met name gegevens coderen in Quantum registers die beknopt zijn ten opzichte van het aantal functies, op systematische wijze Quantum gebruiken entanglement als reken kundige resource en gebruik quantum meting voor klasse-demijner.
...
Classificatie is een machine learning taak onder Super visie, waarbij het doel heeft om klassen labels te leiden $\{y_1, y_2, \ldots, y_d\}$ van bepaalde gegevens voorbeelden. De ' training data set ' is een verzameling voor beelden van $ \mathcal{D} =\{(x, y)} $ met bekende vooraf toegewezen labels. Hier $x $ een gegevens voorbeeld is en $y $ is het bekend label met de naam training label.
