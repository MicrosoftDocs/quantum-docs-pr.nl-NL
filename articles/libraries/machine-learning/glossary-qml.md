...
### <a name="learning-rate"></a>Leer tempo
...
De QML-bibliotheek is de beste manier om de Hyper parameters te wijzigen door de standaard waarden van de UDT- [`TrainingOptions`](xref:microsoft.quantum.machinelearning.trainingoptions)te overschrijven. Hiertoe roept u deze aan met de functie [`DefaultTrainingOptions`](xref:microsoft.quantum.machinelearning.defaulttrainingoptions) en past u de operator `w/` toe om de standaard waarden te onderdrukken. Als u bijvoorbeeld 100.000-metingen en een leer tempo van 0,01 wilt gebruiken:
 ```qsharp
