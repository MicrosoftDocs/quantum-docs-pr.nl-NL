...
Het modelleren van kwantumsystemen met behulp van kwantumprogramma's biedt enorm veel mogelijkheden voor het verkrijgen van inzichten die leiden tot innovaties in allerlei branches. Fotosynthese, supergeleiders en complexe moleculen zijn voorbeelden van kwantumsystemen die kunnen worden gesimuleerd met behulp van kwantumprogramma's.

Het simuleren van microscopische systemen die zich gedragen op basis van de wetten van de kwantummechanica, kost enorm veel rekenkracht. We moeten rekening houden met alle mogelijke toestanden die zich in superpositie kunnen bevinden en het aantal toestanden neemt exponentieel toe met de grootte van het systeem. In een kwantumcomputer hoeven we niet alle toestanden van het systeem te modelleren. In plaats daarvan sluiten we de kwantumtoestand van het systeem dat we willen simuleren in de qubits van de computer zelf in en voeren we de simulatie uit met een specifieke set kwantumgates. Met andere woorden, we gebruiken een kwantumcomputer om een kwantumsysteem te simuleren.

Chemische moleculen zijn kwantumsystemen en kunnen daarom op deze manier worden geanalyseerd. Een voorbeeld van een dergelijke specifieke chemische stof is het enzym _nitrogenase_. Als we een beter inzicht verkrijgen in de eigenschappen van deze stof, kan deze potentieel geschikt zijn om het energieverbruik en de uitstoot van broeikasgassen door bestaande meststoffen te verminderen.
...
## <a name="quantum-inspired-computing"></a>Door kwantum geïnspireerde computing

Door kwantum geïnspireerde algoritmen worden geïmplementeerd met klassieke software, maar gebruiken kwantumprincipes voor hogere snelheid en verbeterde nauwkeurigheid.

Door kwantum geïnspireerde algoritmen worden toegepast in medisch onderzoek, bijvoorbeeld om de nauwkeurigheid van MRI-scans te verbeteren. Door kwantum geïnspireerde computing wordt gebruikt om de configuratie van de MRI-apparatuur te optimaliseren voor het identificeren van specifieke ziekten.
