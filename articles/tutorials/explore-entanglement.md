---
title: 'Verken entanglement met :::no-loc(Q#):::'
description: 'Meer informatie over het schrijven van een Quantum programma in :::no-loc(Q#)::: . Een toepassing voor een Bell-toestand ontwikkelen met behulp van de Quantum Development Kit (QDK)'
author: geduardo
ms.author: v-edsanc
ms.date: 05/29/2020
ms.topic: tutorial
...
<span data-ttu-id="59ac2-105">In deze zelf studie laten we u zien hoe u een :::no-loc(Q#)::: programma schrijft dat qubits bewerkt en meet en de effecten van superpositie en entanglement toont.</span><span class="sxs-lookup"><span data-stu-id="59ac2-105">In this tutorial, we show you how to write a :::no-loc(Q#)::: program that manipulates and measures qubits and demonstrates the effects of superposition and entanglement.</span></span>
...
<span data-ttu-id="59ac2-112">U kunt ook de oplossing volgen zonder de QDK te installeren en de overzichten van de :::no-loc(Q#)::: programmeer taal en de eerste concepten van quantum computing te bekijken.</span><span class="sxs-lookup"><span data-stu-id="59ac2-112">You can also follow along with the narrative without installing the QDK, reviewing  the overviews of the :::no-loc(Q#)::: programming language and the first concepts of quantum computing.</span></span>
...
> * <span data-ttu-id="59ac2-116">Laat Quantum Entanglement zien met een :::no-loc(Q#)::: programma dat in een simulator wordt uitgevoerd.</span><span class="sxs-lookup"><span data-stu-id="59ac2-116">Demonstrate quantum entanglement with a :::no-loc(Q#)::: program run in a simulator.</span></span> 
...
<span data-ttu-id="59ac2-128">We zijn nu klaar om te laten zien hoe :::no-loc(Q#)::: Dit gedrag afspeelt.</span><span class="sxs-lookup"><span data-stu-id="59ac2-128">Now, we're ready to demonstrate how :::no-loc(Q#)::: expresses this behavior.</span></span>  <span data-ttu-id="59ac2-129">U begint met het eenvoudigst mogelijke programma en bouwt dit op om kwantumsuperpositie en kwantumverstrengeling te demonstreren.</span><span class="sxs-lookup"><span data-stu-id="59ac2-129">You start with the simplest program possible and build it up to demonstrate quantum superposition and quantum entanglement.</span></span>

## <a name="creating-a-no-locq-project"></a><span data-ttu-id="59ac2-130">Een :::no-loc(Q#)::: project maken</span><span class="sxs-lookup"><span data-stu-id="59ac2-130">Creating a :::no-loc(Q#)::: project</span></span>

<span data-ttu-id="59ac2-131">Het eerste wat u moet doen, is het maken van een nieuw :::no-loc(Q#)::: project.</span><span class="sxs-lookup"><span data-stu-id="59ac2-131">The first thing we have to do is to create a new :::no-loc(Q#)::: project.</span></span> <span data-ttu-id="59ac2-132">In deze zelf studie gaan we de omgeving gebruiken op basis van [ :::no-loc(Q#)::: toepassingen met VS code](xref:microsoft.quantum.install.standalone).</span><span class="sxs-lookup"><span data-stu-id="59ac2-132">In this tutorial we are going to use the environment based on [:::no-loc(Q#)::: applications with VS Code](xref:microsoft.quantum.install.standalone).</span></span>
...
1. <span data-ttu-id="59ac2-134">Klik op **Weergave** -> **Opdrachtpallet** en selecteer **:::no-loc(Q#):::: Nieuw project maken** .</span><span class="sxs-lookup"><span data-stu-id="59ac2-134">Click **View** -> **Command Palette** and select **:::no-loc(Q#):::: Create New Project** .</span></span>
...
#### <a name="about-no-locq-operations"></a><span data-ttu-id="59ac2-157">Over :::no-loc(Q#)::: bewerkingen</span><span class="sxs-lookup"><span data-stu-id="59ac2-157">About :::no-loc(Q#)::: operations</span></span>

<span data-ttu-id="59ac2-158">Een :::no-loc(Q#)::: bewerking is een Quantum subroutine.</span><span class="sxs-lookup"><span data-stu-id="59ac2-158">A :::no-loc(Q#)::: operation is a quantum subroutine.</span></span> <span data-ttu-id="59ac2-159">Dat wil zeggen dat het een aanroep bare routine is die aanroepen naar andere Quantum bewerkingen bevat.</span><span class="sxs-lookup"><span data-stu-id="59ac2-159">That is, it is a callable routine that contains calls to other quantum operations.</span></span>
...
<span data-ttu-id="59ac2-161">Het retourtype van de bewerking wordt na een dubbele punt opgegeven.</span><span class="sxs-lookup"><span data-stu-id="59ac2-161">The return type of the operation is specified after a colon.</span></span> <span data-ttu-id="59ac2-162">In dit geval heeft de `SetQubitState` bewerking geen retour type, zodat deze als retour waarde wordt gemarkeerd `Unit` .</span><span class="sxs-lookup"><span data-stu-id="59ac2-162">In this case, the `SetQubitState` operation has no return type, so it is marked as returning `Unit`.</span></span> <span data-ttu-id="59ac2-163">Dit is het :::no-loc(Q#)::: equivalent van `unit` in F #, wat ongeveer hetzelfde is als `void` in C# en een lege tuple in python ( `()` , vertegenwoordigd door de type hint `Tuple[()]` ).</span><span class="sxs-lookup"><span data-stu-id="59ac2-163">This is the :::no-loc(Q#)::: equivalent of `unit` in F#, which is roughly analogous to `void` in C#, and an empty tuple in Python (`()`, represented by the type hint `Tuple[()]`).</span></span>

<span data-ttu-id="59ac2-164">U hebt twee Quantum bewerkingen gebruikt in uw eerste :::no-loc(Q#)::: bewerking:</span><span class="sxs-lookup"><span data-stu-id="59ac2-164">You have used two quantum operations in your first :::no-loc(Q#)::: operation:</span></span>
...
<span data-ttu-id="59ac2-188">In beide gevallen wordt het type variabele door de compiler afgeleid.</span><span class="sxs-lookup"><span data-stu-id="59ac2-188">In both cases, the type of a variable is inferred by the compiler.</span></span> <span data-ttu-id="59ac2-189">:::no-loc(Q#)::: geen type annotaties voor variabelen vereist.</span><span class="sxs-lookup"><span data-stu-id="59ac2-189">:::no-loc(Q#)::: doesn't require any type annotations for variables.</span></span>
...
<span data-ttu-id="59ac2-224">Laten we nu eens kijken hoe :::no-loc(Q#)::: u entangle qubits.</span><span class="sxs-lookup"><span data-stu-id="59ac2-224">Now let’s look at how :::no-loc(Q#)::: expresses ways to entangle qubits.</span></span>
